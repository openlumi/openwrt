...
function handleZigbeeFw(ev)
{
	return new Promise(function(resolveFn, rejectFn) {
		var cmd = ev.target.getAttribute('data-command'),
		    ...
		    url = 'admin/system/smptools/flash/' + encodeURIComponent(cmd);

		var dlg = L.showModal(_('Executing zigbee firmware updater'), [
			E('p', { 'class': 'spinning' },
				_('Waiting for the <em>smpflash %h</em> command to complete…').format(cmd))
		]);
		...
		L.post(url, { firmware: fw }, function(xhr, res) {
			...
			if (res.code !== 0)
				dlg.appendChild(E('p', _('The <em>smpflash %h</em> command failed with code <code>%d</code>.').format(cmd, (res.code & 0xff) || -1)));
			...
			dlg.appendChild(E('div', { 'class': 'right' },
				E('div', {
					...
					'click': L.bind(function(res) {
						...
						if (res.code !== 0)
							rejectFn(new Error(res.stderr || 'smpflash error %d'.format(res.code)));
						else
							resolveFn(res);
					}, this, res)
				}, _('Dismiss'))));
